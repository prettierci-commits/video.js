...
test("running message printed", function() {
  ...
  equal(
    this.fakeElem.innerHTML,
    expected,
    "innerHTML was set correctly by testStart"
  );
});
...
test("test expansions are hidden", function() {
  QUnit.testDone();
  equal(
    this.fakeElem.children[0].style.display,
    "none",
    "first ol display is none"
  );
  equal(
    this.fakeElem.children[1].style.display,
    "none",
    "second ol display is none"
  );
});
...
test("non-ol elements aren't hidden", function() {
  ...
  QUnit.testDone();
  notEqual(
    this.fakeElem.children[2].style.display,
    "none",
    "first ol display is none"
  );
});
        ...
        var setAttributeCall = (this.setAttributeCall = {}),
          ...
          fakeElement = {
            appendChild: function() {
              appendChildCall.called++;
            },
            setAttribute: function() {
              setAttributeCall.args = arguments;
            },
            addEventListener: function(type, callback) {
              iframeLoad.callback = callback;
            },
