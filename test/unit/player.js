...
test("should accept options from multiple sources and override in correct order", function() {
  ...
  ok(
    player1.options_["attr"] === "asdf",
    "Tag options overrode global options"
  );
  player1.dispose();
  ...
  ok(
    player2.options_["attr"] === "fdsa",
    "Init options overrode tag and global options"
  );
  player2.dispose();
});
...
test("should get tag, source, and track settings", function() {
  ...
  var html =
    '<video id="example_1" class="video-js" autoplay preload="metadata">';
  ...
  ok(
    player.el().className.indexOf("video-js") !== -1,
    "transferred class from tag to player div"
  );
  ok(player.el().id === "example_1", "transferred id from tag to player div");

  ok(
    vjs.players[player.id()] === player,
    "player referenceable from global list"
  );
...
test("should transfer the poster attribute unmodified", function() {
  ...
  equal(
    player.tech.el().poster,
    poster,
    "the poster attribute should not be removed"
  );
...
test("should load a media controller", function() {
  ...
  ok(
    player.el().children[0].className.indexOf("vjs-tech") !== -1,
    "media controller loaded"
  );
...
test("should toggle user the user state between active and inactive", function() {
  ...
  ok(
    player.el().className.indexOf("vjs-user-active") === -1,
    "Active class removed"
  );
  ok(
    player.el().className.indexOf("vjs-user-inactive") !== -1,
    "Inactive class added"
  );
...
test("should add a touch-enabled classname when touch is supported", function() {
  ...
  ok(
    player.el().className.indexOf("vjs-touch-enabled"),
    "touch-enabled classname added"
  );
...
test("should allow for tracking when native controls are used", function() {
  ...
  ok(
    player.el().className.indexOf("vjs-using-native-controls") !== -1,
    "Native controls class added"
  );
