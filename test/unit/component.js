...
test("should do a deep merge of child options", function() {
  ...
  ok(
    children["childOne"]["foo"] === "baz",
    "value three levels deep overridden"
  );
  ok(
    children["childOne"]["asdf"] === "fdsa",
    "value three levels deep maintained"
  );
  ...
  ok(
    vjs.Component.prototype.options_["example"]["childOne"]["foo"] === "bar",
...
test("should use a defined content el for appending children", function() {
  ...
  ok(
    comp.el().childNodes[0]["id"] === "contentEl",
    "Content El should still exist"
  );
  ok(
